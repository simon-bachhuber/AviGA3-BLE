...
//Read Register
int8_t bmi270_readRegister(uint8_t reg_addr, uint8_t *data, uint16_t len)
{
	//Generating Buffer
	uint8_t rxarr[SPI_BUFF_SZ] = {reg_addr};

	//SPI: CS-0
	HAL_GPIO_WritePin(SPI_CS_BMI_GPIO_Port, SPI_CS_BMI_Pin, GPIO_PIN_RESET);

	//SPI: Recieve the data from specified register (Includes dummy byte)
	HAL_SPI_TransmitReceive(&hspi1, rxarr, rxarr, len + 2, 100);

	//SPI: CS-1
	HAL_GPIO_WritePin(SPI_CS_BMI_GPIO_Port, SPI_CS_BMI_Pin, GPIO_PIN_SET);

	//Copy revieved data in array
...
//Write Register
int8_t bmi270_writeRegister(uint8_t reg_addr, uint8_t *data, uint16_t len)
{
	//Generating Buffer
	uint8_t txarr[SPI_BUFF_SZ] = {reg_addr};

	//Copy transmit command/data in array
	...
	HAL_SPI_Transmit(&hspi1, txarr, len + 1, 1000);
...
BMI2_INTF_RETURN_TYPE bmi2_spi_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t len, void *intf_ptr)
{
    ...
    return bmi270_readRegister(reg_addr, reg_data, (uint16_t) len);
}
...
BMI2_INTF_RETURN_TYPE bmi2_spi_write(uint8_t reg_addr, const uint8_t *reg_data, uint32_t len, void *intf_ptr)
{
    ...
    return bmi270_writeRegister(reg_addr, (uint8_t*)reg_data, (uint16_t)len);
}
